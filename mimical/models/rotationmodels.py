...
def mario_rotation(images, angles, base_x=None, base_y=None, utilnum=None):
    ...
    return (fluxes.permute(2, 1, 0) /
            torch.sum(fluxes, (1, 2))).permute(2, 1, 0)


def nearest_neighbour_rotation(images, angles, base_x=None, base_y=None,
                               utilnum=None):
    """
...
def intersection_rotation(images, angles, base_x=None, base_y=None,
                          utilnum=None):
    """
    Fully vectorised image-cube rotation function that rotates via a matrix
    ...
    def weight_neighbours_circleintersection(origin_coords, expanded_origin):
        """
        Takes in (Nfilt, 2, dIM1, dIM2) for origin_coords, describing the float
        values of input pixel coords before rotation to output. Takes in
        (Nfilt, 2, dIM1, dIM2, 3, 3) for expanded_origin, describing the
        integer input pixel coords in the local 3X3 region before rotation to
        output. Calculates the separation of each pixel in expanded origin with
        the origin coordinate and calculates the circluar intersection, then
        normalises to 1. Returns (Nfilt, 2, dIM1, dIM2, 3, 3).
        """
    ...
    weights = weight_neighbours_circleintersection(origin_coords,
                                                   expanded_origin)
    ...
    fluxes_norm = (fluxes.permute(2, 1, 0) / torch.sum(fluxes, (1, 2)))

    return (fluxes_norm).permute(2, 1, 0)
